...
public class ConnectGame {
	...
	public ConnectGame(int width, int height, int min, int max, Random rand) {
		...
		grid = new Grid(width, this. height);
	}
	...
	public Tile getRandomTile() {
		// TODO
		Tile randomizedTile = new Tile(	rand.nextInt(min,max));
		return randomizedTile;
	...
	public void radomizeTiles() {
		...
		for(int y = height-1; y>=0; y--) {
			for(int x = width-1; x>=0; x--) {
				Tile tln = getRandomTile();
				tln.setLocation(x, y);
				grid.setTile(tln,x, y);
	...
	public void tryContinueSelect(int x, int y) {
		...
		if (startTile) {
			...
			int a = selected.size();
			Tile b = selected.get(	a -1	);
			int c =b.getLevel();
			
			if(!selected.contains(selec)) {
				if(selec.getLevel() == selected.get(selected.size()-1).getLevel() && isAdjacent(selec, selected.get(selected.size()-1))) {
					// if level of input , and the level of the previous one is the same
					selec.setSelect(true);
					selected.add(selec);
					sameSelectCount++;
					
				}else if( selec.getLevel() - selected.get(selected.size()-1).getLevel() == 1 && sameSelectCount>1 && isAdjacent(selec, selected.get(selected.size()-1))) {
					// if the level of input and the level of previous one == 1
					// and the user has selected the same level twice
					// because the level is different then the user has to select the same level again twice
					selec.setSelect(true);
					selected.add(selec);
					sameSelectCount = 0;
				}
	...
	public boolean tryFinishSelection(int x, int y) {
		...
		if (startTile) {
			//do
			tryContinueSelect(x,y);
			Tile selec = grid.getTile(x, y);
			
			if(!selected.contains(selec)) {
				if( selected.size() <=1) {
					selected.clear();
					selected.get(0).setSelect(false);
					startTile = false;
					return false;
					...
					for(int i = selected.size(); i>0 ; i--) {
						selected.get(i-1).setSelect(false);
					}
					selected.clear();
					startTile = false;
					return true;
	...
	public void upgradeLastSelectedTile() {
		// TODO
		int lastIndex = selected.size()-1;
		int getLevel = selected.get(lastIndex).getLevel();
		...
		selected.get(lastIndex).setLevel(getLevel + 1);
		
		dialogListener.showDialog("New Block "+ (2^getLevel) + " removing blocks " + (2^min));
		...
		for( Tile t : selected) {
			...
			for(int i = y; i>=0;i--) {
				if(i == 0) {
					...
					grid.setTile(getRandomTile(), x, i);

				}else if (i!=0) {
					Tile temp = grid.getTile(x, i);
					Tile top = grid.getTile(x, i-1);
					grid.setTile(top,x, i);
					grid.setTile(temp, x, i-1);
				}
			}
