...
public class ThreeCushion {
	...
	public void cueBallImpactCushion() {
		...
		if (shotTaken == true && !gameOver) { // checks if the game is over, and if the shot has taken place
			numberOfImpactCushion += 1;
			lastHitBall = 0;

			if (!firstHitRed && inning == 1) { // if the first hit on the first inning is false, then its a foul
				foul();
	...
	public void cueBallStrike(BallType ball) {
		...
		if (shotTaken && !gameOver) { // checks if the game is over, and if the shot has taken place

			if (ball == RED) { //if the ball is red then redHit ++1, used for tracking the different balls requirement for the bank shot
				redHit += 1;

				if (numberOfImpactBall == 0 && numberOfImpactCushion == 0) { // the first ball hit is red, if there is no cushion impact, and no prior ball impact.
					firstHitRed = true;
				}

			}
			if (!firstHitRed && inning == 1) {//if the first(inning =1) ball hit is not red then its a foul
				foul();
	...
	public void cueStickStrike(BallType ball) {
		...
		inningStart = false;
		if (!gameOver && lagWinnerVerif) { // checks if the game is over, and if the shot has taken place
			...
			if (shotTaken == false) { //if this shot is taken twice in an inning without the end shot, then its a foul
				if (currentBall == ball) {// if the player hits the correct ball
					shotTaken = true;
					inningStart = true;
					calledFoul = false;
					// shot has taken place, inning started, and fouls can be triggered
	...
	public void endShot() {
		...
		if (!gameOver && lagWinnerVerif) { // checks if the game is over, and if the lag winner has chosen a ball
			if (shotTaken) { //checks if the shot has been taken
				shotTaken = false; 

				if (currentTurn == PLAYER_A) { //Checks for PlayerA's variables
					if (numberOfImpactCushion >= 3 && numberOfImpactBall >= 2 && lastHitBall >= 1) { // these are the requirements of a score
						PLAYER_A_POINTS += 1;
						if (PLAYER_A_POINTS == pointsToWinGame) {//if the score has increased to the point where the game is won
							gameOver = true;
						}
						...
						if (!calledFoul) { // checks if a foul can be triggered so that turns only change once
							foul();
						}
					}
				} else if (currentTurn == PLAYER_B) { //Checks for Player B variables

					if (numberOfImpactCushion >= 3 && numberOfImpactBall >= 2 && lastHitBall >= 1) { // these are the requiremetns of a score
						PLAYER_B_POINTS += 1;
						if (PLAYER_B_POINTS == pointsToWinGame) { //if the score has increased to the point where the game is won
							gameOver = true;
						}
	...
	public void foul() {
		...
		if (!gameOver && lagWinnerVerif) { // checks if the game is over, and if the shot has taken place
			if (currentTurn == PLAYER_A) { // changes variables for player A
				currentTurn = PLAYER_B;
				currentBall = PLAYER_B_BALL;
				inning += 1;
				
				//changes the current player and ball to the other player, inning changes +=1
			} else if (currentTurn == PLAYER_B) { // changes variables for player B
			...
			// inning stopped if a foul happens, and foul has been called, so end shot cannot change the player
		}
	...
	public boolean isBankShot() {
		boolean res = false;
		if (numberOfImpactCushion >= 3 && lastHitBall >= 2 && redHit == 1) { // these are the requirements of a bank shot
			res = true;
		}
	...
	public boolean isBreakShot() {
		...
		if (inning == 1 && shotTaken) { //if the inning is 1, and the shot has been taken, then it is a breakshot
			res = true;
		}
		return res;
	}
	...
	public void lagWinnerChooses(boolean selfBreak, BallType cueBall) {
		...
		if (selfBreak) {
			...
			if (lagWinner1 == PLAYER_A) { //sets variables in respect to player A
				...
				currentBall = PLAYER_A_BALL;
			} else if (lagWinner1 == PLAYER_B) { //sets variables in respect to player A
			...
			if (lagWinner1 == PLAYER_A) { //sets variables in respect to player B, as player A chooses to not break
				currentTurn = PLAYER_B;
				...
				currentBall = PLAYER_B_BALL;
			} else if (lagWinner1 == PLAYER_B) { // //sets variables in respect to player A, as player B chooses not to break
				currentTurn = PLAYER_A;
